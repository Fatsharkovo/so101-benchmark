...
## 2026-09-19 — 方块左右范围再收窄 2 cm

- 入盘及全部叠块任务的方块中心左右范围从 ±15 cm 改为 ±13 cm，总宽度由 30 cm 变为 26 cm。
- 保留全区域随机摆放、抓取间距、可达半径与盘子 ±18 cm 的范围。

## 2026-09-18 — 缩窄方块左右随机范围

- 入盘和叠块任务的方块中心左右范围由 ±18 cm 缩为 ±15 cm，两侧各内收 3 cm。
- 盘子仍为 ±18 cm；前后范围、可达半径和抓取间距保持原设置。

## 2026-09-18 — 全工作区随机摆放与闭合夹爪初始化

- 遥操作启用 layout.mode=workspace；入盘五块及盘子、全部叠块组合中的两块从同一片
  可抓取区域重新抽取 XY，不再围绕 XML 锚点扰动。区域为前方 12–27 cm、左右 ±18 cm，
  水平半径 19–29 cm；按外形保留 25 mm 抓取间隙，颜色的采样顺序也随机。
- 保留同 seed 可复现、同大类/反向任务布局一致性；旧配置默认 jitter，关闭布局组仍固定。
- 默认 home_degrees 的夹爪从 70 改为 0；首次初始化、重置及当前布局恢复都直接闭合，
  后续 Leader 输入仍可正常打开夹爪。

## 2026-09-18 — 遥操作每次重置重新随机摆放

- R（等待/录制中）、失败、超时和成功后的重置统一递增 seed，重新采样位置并回到 WAITING。
- 录制中重置先等待写入端确认丢弃，再生成新布局；重复 R 或重复确认不会多次递增。
- Space 仍恢复当前 seed 的初始状态并开始采集，不再随机换位置；明确关闭布局随机化
  的配置仍保持固定布局。采集计数只在成功保存确认后增加。
