...
## 方块左右范围缩至 ±13 cm（2026-09-19）

- 工作区布局、遥操作及任务大类相关回归 **44 passed**，覆盖入盘与全部叠块组合。
- 额外抽样 1000 个入盘布局（5000 个方块位置），全部成功生成；实际左右中心坐标
  位于 -12.9989 至 +12.9992 cm，单次采样最慢约 0.079 秒。
- 物体间抓取间距、盘子范围和可达半径保持原设置；Ruff、pre-commit、
  git diff --check 通过。本轮未操作实体硬件。
