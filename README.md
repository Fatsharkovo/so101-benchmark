...
`layout.mode: workspace` 在机械臂基座前方 x=12–27 cm、方块左右 y=±13 cm（盘子 ±18 cm），
且水平距基座 19–29 cm 的区域内重新抽取物体中心坐标，不使用 XML 中的原始 XY 作为随机中心。
入盘任务的五个方块、全部叠块任务的两个方块均使用同一片方块采样区域，各颜色没有固定区域。
